...
class DynRnn:
	...
	def model(self):
		...
		tf.summary.histogram("dec/kernel/grad", tf.get_default_graph().get_tensor_by_name('gradients/while/dec_cell/MatMul/Enter_grad/b_acc_3:0'))
...
if __name__ == '__main__':
	...
	for i in range(_EPOCHS):
		...
		summ, deco, lo, _ = sess.run(
			[my_rnn.merge, my_rnn.dec_outputs, my_rnn.loss, my_rnn.train],
	...
	deco = sess.run([my_rnn.dec_outputs], feed_dict={my_rnn.X: inp[0:2], my_rnn.Y: oup[0:2], my_rnn.batch_size: 2, my_rnn.is_running: True})
